...
def download_file(url, out_file):
    ...
    if not os.path.isfile(out_file):
        print('\t' + url + ' > ' + out_file, datetime.datetime.now())
...
def download_scan(scan_id, out_dir, file_types):
    print('Downloading MP scan ' + str(scan_id) + ' ...')
    if os.path.exists(out_dir):
        print("already donwloaded", scan_id)
        return 0
    ...
    for ft in file_types:
        url = BASE_URL + RELEASE + '/' + str(scan_id) + '/' + ft + '.zip'
        out_file = out_dir + '/' + ft + '.zip'
        if download_file(url, out_file) == -1:
            failed_files.append(str(scan_id))
            print("download failed", scan_id)
            return -1

    print("download success", scan_id)
    return 0
...
def main():
    ...
    if args.id and args.id != 'ALL':  # download single scan
        ...
        with Pool(4) as p:
            print(release_scans)
            p.map(download_release, release_scans)
        # download_release(release_scans, out_dir, file_types)

if __name__ == "__main__":
    with open("/home/jh27kim/code/current_projects/dataset/matterport/failed_files.txt", 'w') as fp:
        fp.write("starting downloading")
